function enviar() {
    ...
    let placeholder = {
        ciudadInput: "[Ciudad]",
        fechaInput: "[Fecha]",
        nombreinputArrendador: "[Nombre Arrendador]",
        dniinputArrendador: "[DNI Arrendador]",
        direccionInput: "[Direccion de la vivienda]",
        nombreinputArrendatario: "[Nombre Arrendatario]",
        dniinputArrendatario: "[DNI Arrendatario]"
    }
        ...
        if (input && claseSpan) {
            ...
            input.addEventListener('input', () => {
                document.querySelectorAll(`.${claseSpan}`).forEach(span => {
                    span.textContent = input.value ? input.value : placeholder[inputId];
                });
            });
        }
    }
